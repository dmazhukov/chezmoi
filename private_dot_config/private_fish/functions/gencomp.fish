function gencomp -d 'generate completions for fish-shell with usage messages'
    ...
    function __gencomp_usage
        ...
        echo "    -U, --usesub       use the specified command to get usage of subcommand"
        echo "                       ``{1'' is replaced with the arguments"
    ...
    function __gencomp_parse -a cmd sub use_command is_subcmd_parse_mode
        ...
        eval (string replace -a -- "{}" "$cmd $sub" "$use_command" | string replace -a -- "{1}" "$use_subcommand") 2>&1 | tr \t ' ' | while read -l line

            # parse subcommand
            if string match -iqr "^([\w ]* )?(sub)?commands?( [\w ]*)?" -- "$line"
                set section command
                continue
            end
            ...
            if test "$section" = command -a -z "$sub"
                ...
                set -l words (string match -r -- '^ +('$cmd')?([\w-]+)(?:, *)\w(?:[,= ] *)(.*)' "$line")
                if string match -- "$words[2]" "$cmd"
                    set -e $words[2]
                end
                ...
                set -l words (string match -r -- '^ +('$cmd')?([\w-]+)(?:[,= ] *)(.*)' "$line")
                if test (count $words) = 3
                    __gencomp_subcommand_completion "$cmd" "$words[2]" "$words[3]"
                    if test "$is_subcmd_parse_mode" = true
                        __gencomp_parse "$cmd" "$words[2]" "$use_command" false
    ...
    argparse -n gencomp -x 'E,e,l' -x 'E,d' -x 'e,d' -x 'l,d' -x 'E,S' -x 'e,S' -x 'l,S' \
        'd/dry-run' 'E-edit' 'e-erase' 'l/list' 'r/root' 'S/subcommands' 'u/use=' 'U/usesub=' 'w/wraps=+' 'h/help' -- $argv
    or return 1
    ...
    set -lq _flag_U
    and set -l use_subcommand $_flag_U
    or set -l use_subcommand '{1}'
